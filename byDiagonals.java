    ...
    public static void main(String[] args) {
        ...
        for (int k = 0; k < n; k++) {
            ...
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    if (i + j == sum)
                        System.out.println("arr[" + i + "]" + "[" + j + "]: " + arr[i][j] + " ");;// display elements above the side and the diagonal itself
        }
        for (int k = 1; k < n; k++) {
            int sum = n - 1 + k;
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    if (i + j == sum)
                        System.out.println("arr[" + i + "]" + "[" + j + "]: " + arr[i][j] + " ");// display side diagonals
        }

        System.out.println("\nAlong the main diagonals:");
        int i = 0;
        for (int j = n - 1; j >= 0; j--) {
            System.out.println("arr[" + i + "]" + "[" + j + "]: " + arr[i][j] + " "); // start at the top-right element
            int k = i;
            int m = j;
            while (arr[k + 1][m + 1] != 0) {
                k++;
                m++;
                System.out.println("arr[" + k + "]" + "[" + m + "]: " + arr[k][m] + " ");
